...
SELECT EOM.DATE_KEY
	  ...
	  ,CASE 
			WHEN EOM.SRC_TYPE_CD = 'AFS' THEN
				   ...
				   ELSE IIF(LEN(EMP.FICO)<1, PL.A15738_MinFICO, TRY_CAST(RIGHT(EMP.FICO,3) AS INT)) END
	  ...
	  ,CASE
	   ...
	   END AS FICO_CURR
	   ,CASE 
			WHEN EOM.SRC_TYPE_CD = 'IL' THEN ILDT.[OIL_LOAN_SCORE_DT]
...
----SELECT DISTINCT LEN([ACCOUNT #]) FROM  CURRENT_FICO_20171231
--	WITH TEMP AS (
...
--SELECT [DATE_KEY], [SRC_TYPE_CD],  [Acct_Nbr], [ORIG_DT], [FICO_ORIG], [FICO_CURR], '', GETDATE()
--  SELECT TOP 1 * FROM #FICO_ALL