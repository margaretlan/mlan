...
SELECT 'FICO_ORIG' AS FIELD, ACCT_NBR, SRC_TYPE_CD, FICO_ORIG AS VALUE, 
	CONVERT(DATE, CONVERT(VARCHAR(20), 20180228)) AS EFFECTIVE_FROM_DT, '' AS EFFECTIVE_TO_DT, 'Original FICO Score Update' AS REASON, 'mlan' AS [USER_ID]
...
SELECT 'FICO_CURR' AS FIELD, ACCT_NBR, SRC_TYPE_CD, FICO_CURR AS VALUE, 
	CONVERT(DATE, CONVERT(VARCHAR(20), 20180228)) AS EFFECTIVE_FROM_DT, '' AS EFFECTIVE_TO_DT, 'Current FICO Score Update' AS REASON, 'mlan' AS [USER_ID]
FROM [SB_PA_Margaret].[FRB\mlan].[TBL_FICO_TEMP]
UNION
SELECT 'FICO_CURR_DT' AS FIELD, ACCT_NBR, SRC_TYPE_CD, CONVERT(INT, CONVERT(DATETIME, FICO_CURR_DT)) AS VALUE, 
	CONVERT(DATE, CONVERT(VARCHAR(20), 20180228)) AS EFFECTIVE_FROM_DT, '' AS EFFECTIVE_TO_DT, 'Current FICO Date Update' AS REASON, 'mlan' AS [USER_ID]
FROM [SB_PA_Margaret].[FRB\mlan].[TBL_FICO_TEMP]
...
INSERT INTO [SB_PA_Margaret].[FRB\mlan].[TBL_WB_EOM] (
  ...
  SELECT [FIELD]
	...
	LEFT JOIN [RDM_Loan].[ADJ].[V_LOAN_GL_RECONCILED_EOM] EOM
		ON EOM.ACCT_NBR = FICO_DT.ACCT_NBR AND EOM.DATE_KEY = 20180228
	WHERE [FIELD] = 'FICO_CURR_DT' AND [VALUE] <> EOM.FICO_CURR_DT


	SELECT CURR.* , TEMP.FICO_CURR FROM FICO_CURR_20171231 CURR
	LEFT JOIN  [SB_PA_Margaret].[FRB\mlan].[TBL_FICO_TEMP] TEMP ON TEMP.ACCT_NBR =  CURR.ACCT_NBR
    WHERE FICO_FINAL = FICO_CURR AND FICO_FINAL IS NOT NULL

	--UPDATE A
	--SET A.FICO_CURR = B.FICO_FINAL
	--FROM [SB_PA_Margaret].[FRB\mlan].[TBL_FICO_TEMP] A
	--LEFT JOIN FICO_CURR_20171231 B ON B.ACCT_NBR =  A.ACCT_NBR
	--WHERE FICO_FINAL <> FICO_CURR AND FICO_FINAL IS NOT NULL 
	-- RUN ON 3/13 AFTER CURRENT FICO LOGIC IS REVISED