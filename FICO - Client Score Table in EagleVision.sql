...
HAVING  ( ( ( [EAGLEVISION].[DBO].[V_AFSXFI_PRIN_INFO_HIST].PROD_DT ) =  MAX([EAGLEVISION].[DBO].[V_AFSXFI_PRIN_INFO_HIST].PROD_DT) ) --(SELECT DISTINCT PROD_DT FROM SUBMISSION) )--'2018-03-16 00:00:00.000' ) 
...
WITH NON_ZERO AS (SELECT * FROM [CreditAdmin].[FRB\mlan].[FICO_CLIENT] WHERE DATE_KEY = 20180228 AND (FICO_ORIG<851 AND FICO_ORIG>299) ) 
...
--CREATE VIEW [Client FICO SCORES] AS 
...
/**--=================CREATE UNIQUE FICO TO TIN/OBLG TABLE. SO EACH TIN/OBLG WILL HAVE A UNIQUE FICO ASSIGNED===================================================
  ...
  select ssn, count(ssn), min(FICO_ORIG) ficomin, max(FICO_ORIG) ficomax, avg(FICO_ORIG) ficoavg, count( distinct  [FICO_ORIG]) fico#
  FROM [CreditAdmin].[FRB\mlan].[FICO_CLIENT_NONZERO] 
  group by ssn

   select tin, count(tin), min(FICO_ORIG) ficomin, max(FICO_ORIG) ficomax, avg(FICO_ORIG) ficoavg, count( distinct  [FICO_ORIG]) fico#
  FROM [CreditAdmin].[FRB\mlan].[FICO_CLIENT_NONZERO] 
  group by tin
**/